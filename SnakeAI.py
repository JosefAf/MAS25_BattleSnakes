...
class SnakeAI:
    ...
    def update_state(self, game_state):
        ...
        # get obstacle grid
        self.get_Obstacle_Grid(True)  #True parameter gives debug print of grid
    ...
    def get_Obstacle_Grid(self, debug: bool = False):
        ...
        grid = np.zeros((H, W), dtype=np.uint8)  #storing the type numpy.uint8 - unsigned 8-bit integer (0 to 255)
        ...
        my_len = len(me['body'])
        my_nei = {(mx + dx, my + dy) for dx, dy in DIRS
        ...
        for sid, s in self.snakes.items():
            if sid == self.my_snake_id:
                continue
            ox, oy = s['body'][0]['x'], s['body'][0]['y']  #other snake's head position
            ...
            for dx, dy in DIRS:
                ...
                if 0 <= nx < W and 0 <= ny < H and (ox + dx,
                                                    oy + dy) != other_nec:
                    grid[ny, nx] = 1  #assign value 1 (blocked) to this position in grid
                    other_nei.add((nx, ny))
    ...
    def get_Dijkstra(self, start_pos, goal_pos):
        ...
        open_set = []  #List representing open set/ priority queue. Used as heapq (min-heap, lowest prio-value first)
        heapq.heappush(open_set, (0, start_pos))  #Push start position to open set with priority 0
        came_from = {}  #For reconstructing path, this is a dict of child cell (key) - parent cell (value) pairs
        g_scores = {start_pos: 0}  #Cost from start to current node, cost to come per pos in dict
        ...
        while open_set:
            ...
            if current_pos == goal_pos:
                ...
                while current_pos in came_from: #True when current_pos is key in came_from dict
                    current_pos = came_from[current_pos]  #This gives the parent cell of current
                ...
                return len(path)-1  #Distance from start to goal is len(path)-1 since path contains both start pos and goal pos
            ...
            for neighbor in self.get_Neighbors(current_pos):
                ...
                preliminary_g_score = g_scores[current_pos] + 1 #preliminary, because we might've found a better g_score before
                #if neighbor not visited before or if we now have found a shorter path than previously, to same neighbor:
                if neighbor not in g_scores or preliminary_g_score < g_scores[neighbor]:
                    came_from[neighbor] = current_pos
                    g_scores[neighbor] = preliminary_g_score
                    f_score = preliminary_g_score + self.get_Manhattan(neighbor, goal_pos)
